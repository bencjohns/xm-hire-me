    ...
    html.Div([
        ...
        html.Div([ # Standard text container for the block
            ...
            dcc.Markdown("""
                ...
                * *So why NOT get familiar with the survey tools Edge Value Managers discuss with clients daily?*
                ...
                * Learn a LOT, avoid dependence on Red Bull (definitely *not* addicted)
        ...
        html.Div([
            ...
            dcc.Markdown("""
                ...
                This intervention, in section 5 - had Test Group participants being shown a demonstration of the hands-free technology, while Control Group participants were not shown it. As mentioned in the Explanation column in Section 6, the Control Group served to establish a comparison baseline to determine the effects of the feature explanation, allowing me to test my hypothesis that willingness to pay *could likely* increase with a brief feature explanation (in this case, it was purely a text-based "conceptual" explanation given the limitations of our simulation).

                ***[Definition](https://www.qualtrics.com/marketplace/gabor-granger-pricing-sensitivity-study/):*** *a pricing technique used to determine the relationship between pricing and demand (willingness to pay). This relationship is expressed through a price elasticity curve, which we’ve modeled in the interactive chart titled: “Willingness to Pay: Pre vs. Post Feature Explanation (All Segments).” This curve is generated by asking respondents how much they’d be willing to pay at each price point and also determines the revenue-maximizing point for a given product or service.*
            """, className="text-content")
        ...
        html.Div([
            ...
            dcc.Markdown("""
                ...
                **Why choose 15%?**
                ...
                Identifying the specific WTP for Kizik's HandsFree tech would require extensive, in-field market research. However, the 15% average uplift used in this study aligns well with general findings from pricing strategy best-practice principles, conjoint analysis studies across consumer goods, and general benchmarks for convenience/performance-oriented product features. Research indicates that significant, well-communicated innovations can often justify premiums in the 10-40% range, with tangible convenience/performance features narrowing the range down to 10-25%, especially in competitive markets like footwhere where consumers have established price baselines. *Gemini/Claude suggested this lift.*
        ...
        html.Div([
            ...
            dcc.Markdown("""
                ...
                ***[Definition](https://www.qualtrics.com/marketplace/vanwesterndorp-pricing-sensitivity-study/):*** *A kind of price perception survey that helps identify a range of acceptable prices that fit well within customer price perceptions and expectations. It asks four price points: Too Cheap, Bargain, Expensive, Too Expensive.*
            """, className="text-content")
        ...
        html.Div([
            ...
            dcc.Markdown("""
                I added a moderate 20% competitive handicap to willingness to pay estimates that varied by segment (with less reduction for "Premium Enthusiasts" and more for "Value Seekers").

                I chose to add this in after asking about the most glaring errors in the survey, and Claude mentioned that not including it would likely overestimate willingness to pay, given that 1) the survey design at that time asked specifically about Kizik without comparison to alternatives and 2) that real purchase decisions almost always tend to involve trade-offs between brands. Research shows that willingness to pay tends to decrease 15% - 25% when competitive options are presented.

                [ ⚠️ need link to research??? ⚠️ ]
            """, className="text-content")
        ...
        html.Div([
            ...
            dcc.Markdown("""
                ...
                [ ⚠️button to see this snippet of code⚠️ ]

                [ probably need some kind of math equation, will the code be sufficient?? ]

                - Well-established marketing research shows a strong correlation between physical retail presence and brand awareness
                  - Studies by firms like Nielsen and IRI consistently show 15-20% higher brand awareness in areas with physical stores
            """, className="text-content")
        ...
        html.Div([
            ...
            More importantly, it provided to be a quick way to flesh my ideas out. Just like with any other technology, though, it can make mistakes. 
        ...
        html.Div([
            ...
            dcc.Markdown('''
Claude and I explored various survey methodologies, and it suggested using conjoint analysis, which I rejected due to lack of fit.
...
* *Technical Summary by Gemini*
        ...
        html.Div([
            ...
            dcc.Markdown("""
                    **Outcome:** *Feature Explanation Generally Boosts WTP*
                    ...
                    For most segments, being able to receive an “explanation” of the hands-free tech resulted in a higher price point at which 50% pr more of respondents were willing to pay.
            ...
            # --- END COPY THIS BELOW FOR STRONG SECTIONS
                ...
                dcc.Markdown("""
                    ...
                    * Strength/Magnitude of the effect: at 4.53, its coefficient is much less than other that of other predictor variables, namely being in the West, Northeast, etc. So there is a relationship, but it’s just not as *strong* as a relationship between other factors.
        ...
        html.Div([
            ...
            dcc.Markdown("""
                    ...
                    * **Conceptual explanational vs physical:** Real WTP uplift would be captured in an in-person trial
        ...
        html.Div([
            ...
            I used Python, specifically Dash to build this site and Plotly to create the charts. I want to be *very* clear that I don't have a background in computer science and was able to code this line by line all by myself; this was just a lot of trial and error and back and forth with the AI models to build the code and make sure it worked and displayed data accurately.
        ...
        <script>
         window.addEventListener('load', function() {
              // Function to handle arrow rotation using MutationObserver
                ...
                // --- Floating Visualization Toolbar Implementation (ORIGINAL) ---
                     ...
                     function positionToolbar() {
                         try {
                             ...
                             if (availableSpace > toolbarWidth + 32) {
                                 ...
                                 floatingToolbar.style.left = `${sidebarRight + 16}px`;
                             }
                         } catch (e) {
